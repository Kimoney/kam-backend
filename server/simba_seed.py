...
xls_file_path = os.path.expanduser('~/Downloads/SIMBAData.xlsx')
...
with app.app_context():
    ...
    for sheet_name in sheet_names:
        ...
        required_columns = ['YEAR', 'MONTH_', 'ENTRY_NUMBER', 'ENTRYSTATUS', 'REGDATE', 'QUANTITY', 'PORT_OF_DISCHARGE', 'COUNTRYOFORIGIN', 'COUNTRYOFDESTINATION', 'HSCODE', 'DESCRIPTION', 'IMPORT_VAT', 'IMPORT_DUTY', 'EXCISE', "OTHER_TAX"]
        if not all(column in df.columns for column in required_columns):
            raise ValueError(f"Sheet {sheet_name} must contain the following columns: {', '.join(required_columns)}")
        ...
        print(f"Products for sheet {sheet_name} have been populated successfully.")
        ...
        for index, row in df.iterrows():
            ...
            discharge_port = row['PORT_OF_DISCHARGE']
            origin_code = row['COUNTRYOFORIGIN']
            destination_code = row['COUNTRYOFDESTINATION']
            normalized_hs_code_str = row['Normalized_HS_CODE']
            product_name = row['DESCRIPTION']
            ...
            import_declaration_fee = row['OTHER_TAX']
            railway_dev_levy = row['OTHER_TAX'] 
        ...
        print(f"Imports and Taxes for sheet {sheet_name} have been populated successfully.")
